...
(function () {
  ...
  const progressTimer = setInterval(() => {
    if (finished) return;
    const ceiling = imageReady ? 100 : 92;
    const step = imageReady ? 6 : Math.max(0.4, (ceiling - fakeProgress) * 0.08);
    fakeProgress = Math.min(ceiling, fakeProgress + step);
    setBar(fakeProgress);
    if (imageReady && fakeProgress >= 100) finishLoading();
  }, 90);
  ...
  if (bgImage) {
    if (bgImage.complete && bgImage.naturalWidth !== 0) {
      ...
      bgImage.addEventListener('load', markImageReady, { once: true });
      bgImage.addEventListener('error', markImageReady, { once: true });
    }
  } else {
    markImageReady();
  }
...
const playlist = [
  { title: ">_<", src: "nhac1.mp3" },
  { title: ">_<", src: "nhac2.mp3" }
];
let currentTrack = 0;
...
function typeWriterEffect(text, typeSpeed = 320, deleteSpeed = 180) {
  ...
  function loop() {
    if (!isDeleting) {
      ...
      if (i < text.length) {
        ...
        i++;
        typeWriterTimer = setTimeout(loop, typeSpeed);
      } else {
        // gõ xong -> dừng 1400ms rồi xóa
      ...
      if (i > 0) {
        ...
        i--;
        typeWriterTimer = setTimeout(loop, deleteSpeed);
      } else {
        // xóa xong -> dừng 600ms rồi gõ lại
        isDeleting = false;
        typeWriterTimer = setTimeout(loop, 600);
      }
    }
  }
  loop();
...
function loadTrack(index) {
  ...
  typeWriterEffect(playlist[index].title);
...
function getStatusSVG(status, uid) {
  const id = 'm' + uid + '_' + status;
  if (status === 'online') {
    return `<svg viewBox="0 0 32 32" shape-rendering="geometricPrecision"><circle cx="16" cy="16" r="14" fill="#23a55a"/></svg>`;
  }
  if (status === 'idle') {
    // Discord chuẩn: vàng tròn khoét nhỏ góc trên-trái, sừng đỡ nhọn
  ...
  // offline / invisible: vòng xám rỗng ở giữa trong suốt thấy avatar
...
function setDiscordStatus(el, status) {
  if (!el) return;
  const safe = ['online', 'idle', 'dnd', 'offline'].includes(status) ? status : 'offline';
  const baseClass = el.id === 'statusDotDesk' ? 'status-dot' : 'status-dot';
  el.className = baseClass + ' ' + safe;
  el.setAttribute('title', safe);
  const uid = el.id || ('s' + Math.floor(Math.random() * 100000));
  el.innerHTML = getStatusSVG(safe, uid);
}
function initStatusDots() {
  setDiscordStatus(document.getElementById('statusDot'), 'offline');
  setDiscordStatus(document.getElementById('statusDotDesk'), 'offline');
}
...
async function fetchDiscordData() {
  ...
  try {
    ...
    if (data.success && data.data) {
      ...
      setDiscordStatus(document.getElementById('statusDot'), status);
      setDiscordStatus(document.getElementById('statusDotDesk'), status);
